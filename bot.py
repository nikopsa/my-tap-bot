...
from sqlalchemy import Column, BigInteger, Integer, String, DateTime, select, desc
...
async def create_invoice(request: Request):
    d = await request.json()
    prices = {"energy_5k": ["⚡ Энергия 5000", 100], "coins_1m": ["💰 1,000,000 монет", 500]}
    item = prices.get(d['type'])
    link = await bot.create_invoice_link(title=item[0], description="Улучшение", payload=f"pay_{d['type']}_{d['id']}", provider_token="", currency="XTR", prices=[LabeledPrice(label=item[0], amount=item[1])])
    return {"link": link}
...
async def start(m: types.Message):
    async with async_session() as session:
        ...
        if not user: session.add(User(user_id=m.from_user.id, username=m.from_user.username))
        else: user.username = m.from_user.username
        await session.commit()
    await m.answer("Игра готова!", reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[[types.InlineKeyboardButton(text="🔥 ИГРАТЬ", web_app=types.WebAppInfo(url=APP_URL))]]))
